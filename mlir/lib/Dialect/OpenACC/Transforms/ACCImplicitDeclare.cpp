...
// Overview:
// ---------
// Global references in an acc regions (for globals not marked with `acc declare`
// by the user) can be handled in one of two ways:
...
//    and those that represent globals must implement the `acc::GlobalOpInterface`.
//    Additionally, any operations that indirectly access globals must implement
//    the `acc::IndirectGlobalAccessOpInterface`.
//
...
static bool isGlobalUseCandidateForHoisting(Operation *globalOp,
    Operation *user, SymbolRefAttr symbol, acc::OpenACCSupport &accSupport) {
...
static void hoistNonConstantDirectUses(
    AccConstructT accOp, acc::OpenACCSupport &accSupport) {
  accOp.walk([&](mlir::acc::AddressOfGlobalOpInterface addrOfOp) {
    ...
    if (symRef) {
      ...
      if (isGlobalUseCandidateForHoisting(
              globalOp, addrOfOp, symRef, accSupport)) {
...
static void collectGlobalsFromDeviceRegion(mlir::Region &region,
    GlobalOpSetT &globals, acc::OpenACCSupport &accSupport) {
...
static void addDeclareAttr(
    MLIRContext *context, Operation *op, acc::DataClause clause) {
  if (!op) {
    return;
  }
  op->setAttr(acc::getDeclareAttrName(),
      acc::DeclareAttr::get(
          context, acc::DataClauseAttr::get(context, clause)));
}
...
public:
  ...
  void runOnOperation() override {
          ...
          .Case<ACC_COMPUTE_CONSTRUCT_OPS, acc::KernelEnvironmentOp>(
              [&](auto accOp) {
                collectGlobalsFromDeviceRegion(
                    accOp.getRegion(), globalsToAccDeclare, accSupport);
              })
          .Case<mlir::FunctionOpInterface>([&](auto func) {
            if (acc::isAccRoutineOp(func) && !func.isExternal()) {
              collectGlobalsFromDeviceRegion(
                  func.getFunctionBody(), globalsToAccDeclare, accSupport);
            }
          })
          .Case<mlir::acc::GlobalVariableOpInterface>([&](auto globalVarOp) {
            if (globalVarOp->getAttr(acc::getDeclareAttrName())) {
              if (mlir::Region *initRegion = globalVarOp.getInitRegion()) {
                collectGlobalsFromDeviceRegion(
                    *initRegion, globalsToAccDeclare, accSupport);
              }
            }
          })
    ...
    for (auto *globalOp : globalsToAccDeclare) {
      LLVM_DEBUG(llvm::dbgs() << "Global is being `acc declare copyin`d: ";
          globalOp->print(
              llvm::dbgs(), OpPrintingFlags{}.skipRegions().enableDebugInfo());
          llvm::dbgs() << "\n");
